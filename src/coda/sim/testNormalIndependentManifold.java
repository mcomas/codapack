 ...
 * @author mcomas
 */
public class testNormalIndependentManifold {
    ...
    public static void main(String args[]) throws NotPositiveDefiniteMatrixException {
        ...
        double E[] = new double[LOGCONTRAST[0].length];
        E[0] = 1;
        E[1] = 1;
        E[2] = 1;
        E[3] = 1;
        E[4] = 1;
        E[5] = 1;
        double VAR[] = new double[LOGCONTRAST[0].length];
        VAR[0] = 0.5;
        VAR[1] = 0.5;
        VAR[2] = 0.5;
        VAR[3] = 0.5;
        ...
        short n = (short) LOGCONTRAST[0].length;
        
        double a[][] = new double[m+1][n];
        Arrays.fill(a[0], 1);
        for(short i=0;i<m;i++){
            System.arraycopy(LOGCONTRAST[i], 0, a[1+i], 0, n);
        }
        ...
        double base[][] = getKernelBase(new Matrix(a), n);
        ...
        a = new double[dimF+1][n];
        Arrays.fill(a[0], 1);
        for(short i=0;i<dimF;i++){
            System.arraycopy(orthoF[i], 0, a[1+i], 0, n);
        }
        base = getKernelBase(new Matrix(a),n);
        double ilrBase[][] = new double[n-1][n];
        System.arraycopy(orthoF, 0, ilrBase, 0, dimF);
        System.arraycopy(base, 0, ilrBase, dimF, dimG);
        ilrBase = new Matrix(ilrBase).transpose().qr().getQ().transpose().getArray();

        double orthoG[][] = new double[dimG][n];
        for(int i=0;i<dimG;i++)
            orthoG[i] = ilrBase[dimF+i];

        double totalError[][] = new double[1][n];
        for(int i=0;i<n;i++){
            totalError[0][i]=0;
            for(int k=0;k<m;k++) totalError[0][i] += EPS[k] * orthoError[k][i];
        }
        for(int i=0;i<n;i++)
            System.out.println(totalError[0][i]);

        Matrix ILR = new Matrix(ilrBase);
        Matrix ILR_toCLR = ILR.getMatrix(0, n-2, 0, n-2).transpose();
        Matrix CLRtoILR = ILR_toCLR.inverse();
        Matrix ERR = new Matrix(totalError).getMatrix(0, 0, 0, n-2);

        double ilrError[] = CLRtoILR.times(ERR.transpose()).transpose().getArray()[0];
        
/*
        double cov[][] = new double[n][n];
        for(int i=0;i<n;i++)
            for(int j=0;j<n;j++) cov[i][j] = ( i==j? 1 : 1.0);
        for(int step = 1;step < 1000;step++){
            for(int r=0;r<n;r++){
                for(int s=0;s<n;s++){
                    double sum1 = 0;
                    for(int k=0;k<dimF;k++)
                        sum1 += orthoF[k][r] * orthoF[k][s];
                    for(int k=0;k<m;k++){
                        double sum2 = 0;
                        for(int i=0;i<n;i++) for(int j=0;j<n;j++)
                            sum2 += LOGCONTRAST[k][i] * LOGCONTRAST[k][j] * cov[i][j];
                        sum1 += EPS[k] * orthoError[k][r] * orthoError[k][s] * sum2;
                    }
                    cov[r][s] = sum1;
                }
            }
        ...
        for(int i=0; i< SIZE;i++){
            ...
            for(int k=0;k<m;k++) epsilon[k] = ilrError[k] * generator.nextGaussian();
            for(int c=0;c<n;c++){
                double vv = clr[c];
