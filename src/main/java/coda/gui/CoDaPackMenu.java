/**	
 *	Copyright 2011-2016 Marc Comas - Santiago Thi贸
 *
   ...
   public final String ITEM_HELP = "Help";
    ...
    private void crearPersonalMenu(){
        try {
           ...
           if (!archivo.exists()) {
                JOptionPane.showMessageDialog(null,"OJO: 　No existe el archivo de configuraci髇!!");
    ...
    //Definici贸 del LinkedHashMap que far脿 la gesti贸 dels arxius recents
    ...
    public void saveRecentFile(String rf){ 
        ...
        //Obtenim el nom del fitxer amb l'extensi贸, que utilitzarem com a clau
        ...
        //Actualitzada la llista anem a actualitzar el menu i l'acci贸 pertinent
        loadRecentFiles();
    }
    
    //S'encarrega d'escriure el submen煤 Recent Files i definir l'acci贸 pertinent
    public void loadRecentFiles() {
        //Borra tots els items del men煤 Recent Files
        menuRecent.removeAll();
        //Comprovem que hi hagi entrada per posar a Recent Files
        if (newRecentFile.size()>0) {
            //Creem la colecci贸 i l'iterador per a recorrer els valors del LinkedHashMap
            ...
            for (int i=v.length-1;i>=0;i--) {
                ...
                item.setText(v[i]);
                //Definim l'acci贸 a realitzar al clicar l'item
                item.addActionListener(new java.awt.event.ActionListener() {
                    @Override
                    public void actionPerformed(java.awt.event.ActionEvent evt) {
                        JMenuItem jMenuItem = (JMenuItem)evt.getSource();
    ...
    //Aquest m猫tode emplena el LinkedHashMap des de l'arxiu .recent_files cada vegada que s'inicia el programa
    ...
    //Aquest m猫tode sobreescriu l'arxiu .recent_files, quan es tanca el programa i cada vegada que es fa un Clear Items
